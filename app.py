...
def reset_draft():
    if os.path.exists(SAVE_FILE):
        os.remove(SAVE_FILE)
    # Clear session state and go back to home
...
# STAGE 2: DRAFT SETTINGS
elif st.session_state.step == "settings":
    st.title("Draft Settings")
    st.subheader("Configure League Parameters")
    ...
    if st.button("Start Draft", type="primary", use_container_width=True):
        ...
        st.session_state.params = {
            ...
            "DEF": d_req, "MID": m_req, "RUC": r_req, "FWD": f_req
        }
        st.session_state.step = "draft"
        save_state()
        st.rerun()

# STAGE 3: MAIN DRAFT INTERFACE
elif st.session_state.step == "draft":
    p = st.session_state.params # Shortcut
    
    # Sidebar - Simplified (Locked Info + Reset)
    with st.sidebar:
    ...
    # TOP ACTION AREA (SAME INTERFACE AS PREVIOUS)
    ...
    total_required = sum([p['DEF'], p['MID'], p['RUC'], p['FWD']]) + 8 # Core + 2 per pos bench
    ...
    if is_draft_complete:
        ...
        with act_col2:
            ...
            if selected:
                ...
                pos_opts = player_row['positions'].split('/')
