...
if st.session_state.step == "home":
    ...
    with col2:
        ...
        r_r = st.number_input("RUC", value=st.session_state.params["RUC"])
    ...
    if st.button("Start Draft", type="primary", use_container_width=True):
        st.session_state.params = {"num_teams": n_teams, "my_slot": m_slot, "DEF": d_r, "MID": m_r, "RUC": r_r, "FWD": f_r}
    ...
    active_name = get_team_name(active_id)
    
    total_slots = sum([p['DEF'], p['MID'], p['RUC'], p['FWD']]) + 8
    is_complete = len(st.session_state.draft_history) >= (p['num_teams'] * total_slots)

    # Calculate Data for Draft
    taken = [d['player'] for d in st.session_state.draft_history]
    avail_df = df[~df['full_name'].isin(taken)].copy()
    
    # Calculate Recommendations / Opt Score only if draft is active
    if not is_complete and not avail_df.empty:
        active_picks = [d for d in st.session_state.draft_history if d['team'] == active_id]
    ...
    if is_complete:
        ...
        with c2:
            ...
            sel = rc1.selectbox("Select Player:", [""] + sorted(avail_df['full_name'].tolist()), label_visibility="collapsed")
            conf_pos, can_conf = None, False
            if sel:
                pos_o = df[df['full_name'] == sel].iloc[0]['positions'].split('/')
                conf_pos = rc2.radio("Pos:", pos_o, horizontal=True, label_visibility="collapsed")
                if check_roster_limit(conf_pos, active_id, p, st.session_state.draft_history): can_conf = True
            if rc3.button("CONFIRM", type="primary", disabled=not can_conf, use_container_width=True):
                st.session_state.draft_history.append({"pick": curr_p_num, "team": active_id, "player": sel, "assigned_pos": conf_pos})
                save_state(); st.rerun()

        if not avail_df.empty and 'Opt_Score' in avail_df.columns:
    ...
    with tabs[0]:
        if not is_complete:
            ...
            for i, pos in enumerate(['DEF', 'MID', 'RUC', 'FWD']):
                ...
                cols[i].metric(pos, f"{counts[pos]}/{p[pos]}", delta=f"-{rem}" if rem > 0 else "FULL", delta_color="inverse" if rem > 0 else "normal")
        else: st.info("The Draft is complete. View results in the Final Teams tab.")
